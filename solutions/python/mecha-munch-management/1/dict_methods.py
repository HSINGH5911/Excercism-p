...
def update_recipes(ideas, recipe_updates):
    ...
    for recipe, idea in recipe_updates:
        ideas[recipe] = idea
...
def update_store_inventory(fulfillment_cart, store_inventory):
    ...
    for item in fulfillment_cart:
        ordered = fulfillment_cart[item][0]
        available = store_inventory[item][0]

        new_quantitiy = available - ordered

        if new_quantitiy == 0:
            store_inventory[item][0] = "Out of Stock"
        else:
            store_inventory[item][0] = new_quantitiy
